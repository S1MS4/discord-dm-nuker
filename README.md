...
> [!CAUTION] Deletions are permanent. Automating a personal account is against
> [Discord's rules](https://support.discord.com/hc/en-us/articles/115002192352-Automated-User-Accounts-Self-Bots) and
> can result in account termination.

How to use this script:

1. Open Discord in your desktop browser and sign in.
2. Press `Ctrl` + `Shift` + `I` and go to the **Console** tab.
3. Copy the code below, paste it into the console, and press Enter.
4. Select the people you want to remove **your messages** from and click **Delete selected DMs**.
5. Keep Discord open until it finishes. You can close the console.
...
You can also copy the script directly from [discord-dm-panel.js](discord-dm-panel.js).

The counter shows successful deletions. It starts deleting as messages load; you do not have to wait for a full scan.
You can queue more people while it runs.

## FAQ

**It says “Connecting…” and nothing happens.**

Wait a few seconds or open another DM. The script can pick up your session from Discord's next authenticated request. If
it fails, click **Retry**. Never post your token or request headers in an issue.

**Can I close or minimise it?**

Yes. **−** leaves the counter visible. **×** hides the panel and keeps deleting. Paste the script again to reopen it, or
run `dmScrubPanel.show()`. Reloading Discord or closing its tab/app stops the script and loses the queue.

**How do I stop it?**

Click **Pause** or **Stop**. **Resume** continues from the same place while Discord stays open. From the console,
`dmScrubPanel.stop = true` stops requests. `dmScrubPanel.destroy()` stops the script and removes it entirely. An
already-sent request can still finish.

**What does NUKE do?**

**NUKE all DMs** queues every loaded one-to-one DM, including people hidden by the search filter. It hides the panel
when finished. Both delete buttons start deletion immediately, with no second confirmation.

**Does it delete the other person's messages?**

No. Only your ordinary messages and replies, including their attachments. Group DMs, servers, and system entries such as
call records are excluded. DMs missing from Discord's list are not processed.

**Why does it slow down or show `429` in the console?**

Discord is rate limiting the requests. The script waits and retries with longer cooldowns instead of dropping the queue.
It cannot bypass those limits. For other errors, **Retry** appears; hover the counter for the error message.

**A conversation is greyed out, but I sent another message.**

Grey means its scan finished. Newer messages may need another run. Use `dmScrubPanel.destroy()`, then paste the script
again to load a fresh list.

<details>
<summary>Development</summary>

Node.js 22+ is only needed for development. Running the script in Discord requires no installation.
...
Tests use simulated Discord responses and make no live account requests. Private Discord internals can change, so
passing offline tests does not guarantee every client build works.

Edit `discord-dm-panel.js`, then run `npm run docs:sync` to update the code above. CI checks that they match.

</details>

MIT, see [LICENSE](LICENSE). Not affiliated with Discord.
